...
function stopTitration() {
    if(part41done) {
        ...
        if(Math.abs(currentValue - 6.0) <= 0.4) {
            ...
            document.getElementById('divide').innerHTML =  `<b>${currentValue.toFixed(1)} </b>`;

            addTask('Step3: Titrate the mixture with DCPIP dye solution');
            
            
            // Show the text with id="step51" in the taskslist
            // document.getElementById('step51').style.display = 'block';
        }
    }
}
